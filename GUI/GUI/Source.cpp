...
public:
	...
	vector<Tagg*> childeren;
	string text;
	Tagg* parent;

	Tagg(string n, map<string, string> attr, string t)
	{
		name = n;
		attributes = attr;
		text = t;
...
Tagg* addTag(string text, int pos)
{
    // tag attributes
    int index = text.find('>', pos);
    ...
    Tagg* tag = new Tagg(tagName, attrVals, tagText);
    return tag;
}

Tagg* addSelfClosingTag(string text, int pos)
{
...
void printJSON(Tagg* tag, string& text)
{
    // print name
    printIdent(text);
    //cout << "\"" + tag->name + "\":{" << endl;
...
void printXML(Tagg* tag, string& text)
{
    // print name
    printIdent(text);
    text += "<" + tag->name + " ";
...
void formatXML(Tagg* rootTag, string xmlFilePath)
{
...
void formatJOSN(Tagg* rootTag, string JSONfilePath)
{
...
Tagg* buildParseTree(string modifiedSourceFilePath)
{
    Tagg* parseTreeHead = nullptr;
    string mainString;
    ...
    for (int i = 0; i < mainString.size(); i++)
    {
        if (mainString[i] == '<')
        {
            ...
            else if (isSelfClosingTag(mainString, i))
            {
                ...
                Tagg* tag = addSelfClosingTag(mainString, i);
                Tagg* parent = parseTreeHead;
